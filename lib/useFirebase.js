...
export async function joinGroup(id, orgId, groupId) {

  try {
    console.log(await (await usersCollection.where('id','==',id).get()).docs[0].ref.update({[`orgs.${orgId}.groups`]: firestore.FieldValue.arrayUnion(groupId)}))
    console.log(await (await groupCollection.where('id','==',groupId).get()).docs[0].ref.update({members: firestore.FieldValue.arrayUnion(id)}));
    }
...
export async function leaveGroup(id, orgId, groupId) {
  try {
    (await usersCollection
    .where('orgList', 'array-contains', orgId)
    .where('id','==',id)
    .get())
    .docs[0]
    .ref
...
export async function isUserInGroup(groupId, orgId) {
  ...
  try {
    const set = (await groupCollection.where('orgId', '==', orgId).where('id','==',groupId).where('members','array-contains',curUser).get())
    if (set.size>0) {
      console.log('true');
      return true
    } else {
...
export async function getUserAttributes(id, orgId) {
  ...
  try {
    if (id) {
      const groups = await usersCollection.where('id', "==", id).get();
      for (const key of groups.docs) {
        console.log("WOOP");
        console.log(key.data());
        returningArray.push(key.data());
