...
export async function editJob(jobID, role, location, description,method,email,website,deadline) {

  const curUser = CURRuser;
  const docRef = (await jobsCollection.where("id", "==", jobID).get())
    .docs[0].ref;
  const batch = firestore().batch();
  batch.update(docRef, { jobRole: role });
  batch.update(docRef, { location : location });
  batch.update(docRef, { description: description });
  batch.update(docRef, { method: method });
  batch.update(docRef, { email: email });
  batch.update(docRef, { website: website });
  batch.update(docRef, { deadline: deadline });
  await batch.commit();
}
