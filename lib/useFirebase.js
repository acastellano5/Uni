...
export async function getCompanyById(orgId, id) {
  const returningArray = [];
  const reference = await companyCollection
...
export async function createJob(orgId, body) {
  if (body.company) {
    const company = await getCompanyById(orgId, body.company);
    const jobID = getUUID();
    ...
    const compId = ref.get().data().companyID;
    const jobID = getUUID();
    const jobRef = await jobsCollection.add({
      jobRole: body.jobRole,
      company: body.compId,
...
export async function deleteJobs(orgId, id) {
  const job = (await jobsCollection.where("jobID", "==", id).get()).docs[0].ref;
  const compID = (await job.get())
    .data()
    .companyID(
      (
        await companyCollection.where("companyID", "==", compID).get()
      ).docs[0].ref.update({ jobs: firestore.FieldValue.arrayRemove(id) })
    );
  job.delete();
}
