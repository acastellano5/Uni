...
export async function sendAlumniRequest(uid, body, orgId) {
  ...
  try {
    await requestsCollection.add({
      ...
      alumniCollegeValue: body.collegeValue,
    })
  } catch (error) {
    console.log(error);
...
export async function createChat(users, message, orgID) {
  /*
  ...
  */
  ...
  if (users.length == 2) {
    // use userDoc.orgs[orgId].role
    var userRoles = [];
    console.log("orgID: ", orgID);
    for (var user of users) {
      var userRef = await usersCollection.where("id", "==", user.id).get();
      userRoles.push(userRef.docs[0].data().orgs[orgID].role);
    }
    console.log("roles: ", userRoles);
    console.log(userRoles.includes("Student"), userRoles.some(role => role !== "Student"))
    if (userRoles.includes("Student") && userRoles.some(role => role !== "Student")) return "Students can only DM other students. Please add a third user to the chat, or use a third-party service.";
  }
