...
import { fi, id, te } from "date-fns/locale";
const db = firestore();
...
export async function leaveGroup(orgId, groupId) {
  ...
  try {
    ...
    console.log(await (await groupCollection.where('id','==',groupId).get()).docs[0].ref.update({moderators: firestore.FieldValue.arrayRemove(uID)}));
...
export async function isUserModerator(groupId, orgId) {
  ...
  try {
    const set = await groupCollection
      .where('orgId', '==', orgId)
      .where('id', '==', groupId)
      .where('mo', 'array-contains', curUser)
      .get();
...
export async function getOrgs(userId) {
  const returningArray = []
  try {
    const userDoc = (await usersCollection.where('id','==',userId).get()).docs[0].data().orgs
    console.log(userDoc);
