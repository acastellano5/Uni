...
export async function getChats(chatList, filter) {
  var chats = [];
  ...
  for (var chatID of chatList) {
    ...
    for (var i in chat.users) {
      ...
      if (user == auth().currentUser.uid) {
        ...
        chat.users[i] = (userRef.empty) ? "Deleted" : userRef.docs[0].data().fullName;
      }
    }
    
    var lastMsgRef = await chatDoc.ref.collection('Messages').orderBy('timestamp', 'desc').get();
    if (!lastMsgRef.empty) {
      chat.lastMsg = lastMsgRef.docs[0].data();
      if (chat.lastMsg.author == auth().currentUser.uid) {
        chat.lastMsg.author = "You";
      } else {
        var userRef = await usersCollection.where('id', '==', chat.lastMsg.author).get();
        chat.lastMsg.author = (userRef.empty) ? "Deleted" : userRef.docs[0].data().fullName;
      }
    } else {
      chat.lastMsg = null;
    }
  ...
  // order chats by timestamp value
  chats.sort((a, b) => (a.lastMsg && b.lastMsg) ? b.lastMsg.timestamp - a.lastMsg.timestamp : 0);
...
export async function getChat(chatID, addMessage) {
  ...
  chat.unsubscribe = chatDoc.ref.collection('Messages').orderBy('timestamp', 'asc').onSnapshot(async (snapshot) => {
    ...
    for (var msgDoc of snapshot.docChanges()) {
      ...
      if (msg.author == auth().currentUser.uid) {
        msg.author = "You";
      } else {
        var userRef = await usersCollection.where('id', '==', msg.author).get();
        msg.author = (userRef.empty) ? "Deleted" : userRef.docs[0].data().fullName;
      }
      addMessage(msg);
    }
  });
