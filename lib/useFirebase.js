...
export async function joinGroup(orgId, groupId) {
  ...
  try {
    if (groupCollection.where('id',"==", groupId).where('roles','array-contains',role).count>0) {
      console.log(
        await (
...
export async function getUpcomingGroupEvents(groupId, orgId) {
  ...
  try {
    const events = await eventCollection
      .where("orgId", "==", orgId)
      .where("attendees",'array-contains-any',[groupId])
    ...
    for (const key of events.docs) {
      ...
      if (key.data().startTime.seconds >=  seconds) {
...
export async function filterUsers(roleFilter, conditions, orgId) {
  ...
  try {
    ...
    if (roleFilter=="Faculty/Staff") {
      ...
      if (conditions.class) {
        console.log(conditions.department);
      ...
      if (conditions.groups) {
        ...
        queryArray.push(where(`orgs.${orgId}.groups`,'array-contains-any',conditions.groups))
    ...
    if (roleFilter=="Student") {
      ...
      if (conditions[0]) {
        ...
        queryArray.push(where(`orgs.${orgId}.department`,'==',conditions[0]))

      }
      
      if (conditions[1]) {
        console.log(conditions[1]);
        queryArray.push(where(`interests`,'array-contains-any',conditions[1]))

      }
      if (conditions[2]) {
        console.log(conditions[2]);
        queryArray.push(where(`orgs.${orgId}.groups`,'array-contains-any',conditions[2]))
    ...
    if (roleFilter=="Alumni") {
      ...
      if (conditions.college) {
        console.log(conditions.college);
        queryArray.push(where(`interests`,'==',conditions.college))

      }
      if (conditions.field) {
        console.log(conditions.field);
        queryArray.push(where(`orgs.${orgId}.State`,'==',conditions.field))

      }
      if (conditions.interests) {
        console.log(conditions.interests);
        queryArray.push(where(`orgs.${orgId}.Field`,'array-contains-any',conditions.interests))

      }
      if (conditions.state) {
        console.log(conditions.state);
        queryArray.push(where(`orgs.${orgId}.College`,'==',conditions.state))

      }
    }
    console.log(queryArray);
    const q = (await query(usersCollection,...queryArray).get()).size
    console.log(q);
  } catch (error) {
    console.log(error);
    return returningArray
