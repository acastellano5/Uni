...
//_______________________________________________________________
export async function createUserPost(authorType, content, caption) {
  try {
...
export async function createComment(id, content,text) {
  const num = getUUID()
  const postDoc = (await postCollection.where('postId', '==', id).get()).docs[0].ref
  await postDoc.update({[`comments.${num}`]: {author: auth().currentUser.uid, content: content, text: text }})
