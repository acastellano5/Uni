...
export async function getChats(chatList, filter) {
  ...
  for (var chatID of chatList) {
    ...
    var lastMsgRef = await chatDoc.ref.collection('Messages').get();
    if (!lastMsgRef.empty) {
...
export async function getChat(chatID) {
  ...
  for (var i in chat.users) {
    ...
    var messagesRef = await chatDoc.ref.collection('Messages').orderBy('timestamp', 'asc').limit(40).get();
    chat.messages = [];
    for (var msgDoc of messagesRef.docs) {
      var msg = msgDoc.data();
