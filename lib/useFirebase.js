...
export async function createEvent(type, orgId, name, location, startTime, endTime, attendees, authorId, description) {
  ...
  try {
    ...
    if (type=="user") {
      const docId = (await usersCollection.where('uid', '==', authorId).get()).docs[0].ref.update({events: firestore.FieldValue.arrayUnion(uuid)})
  
    }
    if (type=="group") {
      const docId = (await groupCollection.where('id', '==', "e9975f54-7b1f-4539-bbae-9416b8d4b095").get()).docs[0].ref.update({events: firestore.FieldValue.arrayUnion(uuid)})
    }
  } catch (error) {
    console.log(error);
  }
