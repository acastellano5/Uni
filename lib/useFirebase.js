...
export async function leaveGroup(orgId, groupId) {
  ...
  try {
    if (await ifGroupFollowed(groupId,orgId)) {
      ...
      await userDoc.ref.update({[`orgs.${orgId}.groups`]: firestore.FieldValue.arrayRemove(groupId)})
      if (isUserModerator(groupId,orgId)==true) {

        await groupDoc.ref.update({moderators: firestore.FieldValue.arrayRemove(groupId)})
      }
      else{
        console.log(uID);
        await groupDoc.ref.update({members: firestore.FieldValue.arrayRemove(uID)})
      }
    }
  } catch (error) {
    console.log(error);
...
export async function filterUserByRole(orgId,role) {
  ...
  try {
    const filterSet = (await usersCollection.where('orgIdList','array-contains',orgId).get())
...
export async function getGroupsByUser(user,orgId, needData) {
  const returningArray = [];
  try {
    const userDoc = (await usersCollection.where('id', '==', user).get()).docs[0].data()
...
export async function isPostLiked(postId) {
  const user = auth().currentUser.uid
  try {
    const postDoc = (await postCollection.where('postId', '==', postId).where('likes','array-contains',user).get()).docs[0].ref
    return true
  } catch (error) {
    return false
  }
...
export async function getUsers(orgId,name) {
  ...
  try {
    if (name) {
      const setDocs = (await usersCollection.where('orgIdList','array-contains',orgId).get()).docs
...
export async function unAttendEvent(orgId, eventId) {
  ...
  try {
    console.log(eventId);
    const event = (await (await eventCollection.where('eventId','==',eventId).get()).docs[0].data().authorId)

    if (uID==event) {
      const event = (await (await eventCollection.where('eventId','==',eventId).get()).docs[0].ref.delete())
      await (await usersCollection.where('id','==',uID).get()).docs[0].ref.update({events: firestore.FieldValue.arrayRemove(eventId)})

    }
    else{
      console.log(await (await eventCollection.where('eventId','==',eventId).get()).docs[0].ref.update({attendees: firestore.FieldValue.arrayRemove(uID)}));
...
export async function isAttended(eventId, orgId) {
  const user = auth().currentUser.uid
  try {
    if ((await eventCollection.where('eventId', '==', eventId).where('attendees','array-contains',user).get()).docs.length>0) {
      console.log("works");
    }
    return true
  } catch (error) {
...
export async function getOrgs() {
  ...
  try {
    const userDoc = (await usersCollection.where('id','==',userId).get()).docs[0].data().orgIdList
    console.log(userDoc);
...
export async function getFollowers(uid,orgId) {
  const returningArray = []
  try {
    const userDoc = (await usersCollection.where('id','==',uid).get()).docs[0].data().orgs[orgId].social.followers
    for (let index = 0; index < userDoc.length; index++) {
      const element = userDoc[index];
      console.log(element);
      returningArray.push(element)
...
export async function getFollowing(uid,orgId) {
  const returningArray = []
  try {
    const userDoc = (await usersCollection.where('id','==',uid).get()).docs[0].data().orgs[orgId].social.following
    for (let index = 0; index < userDoc.length; index++) {
      const element = userDoc[index];
      console.log(element);
      returningArray.push(element)
...
export async function leaveOrg(orgId) {
  ...
  await (await usersCollection.where('id','==',userId).get()).docs[0].ref.update({orgIdList: firestore.FieldValue.arrayRemove(orgId)})
  for (let index = 0; index < groups.length; index++) {
    const element = groups[index];
  ...
  await (await usersCollection.where('id','==',userId).get()).docs[0].ref.update({orgs: firestore.FieldValue.arrayRemove(orgId)})
