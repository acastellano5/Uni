...
export async function ifGroupFollowed(followingId, orgId, userRole) {
  try {
...
export async function ifUserFollowed(followingId, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    const followedDoc = (
      await usersCollection.where("id", "==", curUser).get()
...
export async function ifUserFollowing(followingId, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    const followedDoc = (
      await usersCollection.where("id", "==", curUser).get()
...
export async function isUserInGroup(groupId, orgId, userRole) {
  const curUser = auth().currentUser.uid;
  try {
...
export async function isUserModerator(groupId, orgId, userRole) {
  const curUser = auth().currentUser.uid;
  try {
    const set = await groupCollection
      .where("orgId", "==", orgId)
...
// done
export async function getGroupById(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
...
export async function getUserByGroup(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
      const groups = await groupCollection
...
// done
export async function getGroupsByName(name, orgId, userRole) {
  const returningArray = [];
  try {
    if (name) {
      const groups = await groupCollection
...
export async function getAllGroups(orgId, userRole) {
  ...
  try {
    console.log("Hi");
    const groups = await groupCollection.where("orgId", "==", orgId).where('roles','array-contains',userRole).get();
    for (const key of groups.docs) {
      console.log(key.data());
      returningArray.push(key.data());
    }
...
// done
export async function getGroupsByCategory(category, orgId, userRole) {
  const returningArray = [];
  try {
    if (category) {
      const groups = await groupCollection
...
// done
export async function getGroupByCategory(category, orgId, userRole) {
  const returningArray = [];
  try {
    if (category) {
      console.log("Hi");
...
export async function getGroupByDescription(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
      console.log("Hi");
...
// need to implement
export async function followGroup(id, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    if (id) {
      const groupDoc = (
...
export async function getUnfollowed(uid, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
...
// need to implement
export async function unfollowGroup(id, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    if (id) {
      const groupDoc = (
...
export async function createUserPost(content, caption, orgId, userRole) {
  let roleset = []
  if (userRole=="student") {
...
export async function createOrgPost(content, caption, orgId, userRole) {
  try {
    const authId = auth().currentUser.uid;
    const docRef = await postCollection.add({
      postId: getUUID(),
...
export async function createGroupPost(groupId, content, caption, orgId, userRole) {
...
export async function getFollowingPosts(orgId, userRole) {
  const user = auth().currentUser.uid;
  const returningArray = [];
  try {
...
export async function getPostsByTime(orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
...
export async function getPostByCaption(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
      console.log("Hi");
...
// need to implement
export async function getPostByGroup(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
      const groups = await postCollection
...
// need to implement
export async function getPostByAuthor(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
...
export async function getPostById(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
...
export async function getComments(postId,orgId, userRole) {
  const returningArray = []
  try {
...
export async function getCommentByUser(user, orgId, userRole) {
  const returningArray = []
  try {
...
export async function getCommentById(id, orgId, userRole) {
  const returningArray = []
  try {
...
export async function getCommentByKeyword(keyword, orgId, userRole) {
  const returningArray = []
  try {
    const posts = (await postCollection.where('orgId', '==', orgId).get()).docs
    for (let index = 0; index < posts.length; index++) {
...
// need to implement
export async function followUser(uid, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    const curUserDoc = usersCollection.where("id", "==", curUser).get();
    const userToFollow = uid;
...
export async function unfollowUser(uid, orgId, userRole) {
  try {
    const curUser = auth().currentUser.uid;
    const curUserDoc = usersCollection.where("id", "==", curUser).get();
    const userToFollow = uid;
...
export async function getUserAttributes(id, orgId, userRole) {
  const returningArray = [];
  try {
    if (id) {
...
export async function getEvents(orgId, userRole) {
  const user = auth().currentUser.uid;
  return getEventByUser(user, orgId);
}
export async function getEventById(id, orgId, userRole) {
  const returningArray = [];
  try {
...
export async function getEventByType(type, orgId, userRole) {
  const returningArray = [];
  try {
    console.log("Hi");
    const events = await eventCollection
...
export async function getEventByName(name, orgId, userRole) {
  const returningArray = [];
  try {
    console.log("Hi");
    const events = await eventCollection
...
export async function getEventByStartTime(startTime, orgId, userRole) {
  const returningArray = [];
  try {
    console.log("Hi");
    const events = await eventCollection
...
export async function getEventByEndTime(endTime, orgId, userRole) {
  const returningArray = [];
  try {
    console.log("Hi");
    const events = await eventCollection
...
export async function getEventByDesc(desc, orgId, userRole) {
  const returningArray = [];
  try {
    console.log("Hi");
    const events = await eventCollection
...
export async function getEventByUser(user, orgId, userRole) {
  const returningArray = [];
  try {
    const events = await eventCollection
      .where("orgId", "==", orgId)
...
export async function getEventByOrg(groupId, orgId, userRole) {
  const returningArray = [];
  try {
...
export async function getCommunityEvents(orgId, userRole) {
  const user = auth().currentUser.uid;
  const returningArray = [];
  try {
...
export async function isAttended(eventId, orgId, userRole) {
  const user = auth().currentUser.uid;
  try {
    if (
      (
...
export async function getFollowers(uid, orgId, userRole) {
  const returningArray = [];
  try {
    const userDoc = (
      await usersCollection.where("id", "==", uid).get()
...
export async function getFollowing(uid, orgId, userRole) {
  const returningArray = [];
  try {
    const userDoc = (
      await usersCollection.where("id", "==", uid).get()
