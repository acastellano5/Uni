...
export async function addCompany(body, orgId) {
  return await companyCollection.add({
    companyID: getUUID(),
...
export async function editCompany(id,body,orgId) {
  const curLogo = (await companyCollection.where('companyID','==',id).get()).docs[0].data().logo
  if (curLogo != body.logo) {
    return await (await companyCollection.where("companyID",'==',id).get()).docs[0].ref.set({
      companyName: body.title,
      owner: auth().currentUser.uid,
      description: body.description,
      location: body.location,
      industry: body.industry,
      logo: await uploadToFirebase(body.logo),
      orgId: orgId
    })
    .then((docRef) => {
      console.log("Job document successfully written with ID: ", docRef.id);
...
export async function deleteCompany(id) {
    return await (await companyCollection.where("companyID",'==',id).get()).docs[0].ref.delete()
    .then((docRef) => {
      console.log("Job document successfully written with ID: ", docRef.id);
    })
    .catch((error) => {
...
export async function getCompanyByIndustry(orgId,industryType) {
  ...
  const reference = await companyCollection.where("industry", "==", industryType).where("orgId", "==", orgId).get();
...
export async function getCompanyByName(orgId,name) {
  const returningArray = [];
  const reference = await companyCollection.where("companyName", 'in', name).where("orgId", "==", orgId).get();
  for (let index = 0; index < reference.size; index++) {
    const element = reference.docs[index].data();
    returningArray.push(element);
  }
...
export async function addJobDetails(body, orgId) {
...
export async function isUserSetup(id) {
  try {
    const docRef = (await usersCollection.where('id','==',id).get()).docs[0].data().accountSetUp
    return docRef
  } catch (error) {
    console.log(error);
    return false
...
export async function editProfile(fullName, bio, interests, image) {
  const imageVar = await uploadToFirebase(image)
  const curUser = CURRuser
...
export async function editGroup(groupDB, groupInfo) {
  try {
    ...
    const curUser = CURRuser
    const docRefD = (await groupCollection.where('id','==',groupDB.id).get()).docs[0].data()
...
export async function getUserOrgs(id) {
  try {
    const set = (await usersCollection.where("id",'==',id).get()).docs[0].data().orgIdList
    if (set) {
      console.log(set);
      return set[0]
    }
...
export async function getDownloadURL(post) {
  ...
  console.log(post);
  console.log('====================================');
  const postObj = (await postCollection.where('postId', '==', post).get()).docs[0].data()
...
export async function getUserChatList() {
  var userData = (await usersCollection.where('id', '==', CURRuser).get()).docs[0].data();
...
export async function getChatPreview(chatID, reloadFunc) {
  ...
  var lastMsgRef = await chatDoc.ref.collection('Messages').orderBy('timestamp', 'desc').limit(1).get();

  chat.lastMsg = lastMsgRef.docs[0].data();
  chat.lastMsg.author = await getCleanName(chat.lastMsg.author);

  // onSnapshot for lastMsg
  chat.unsubscribe = chatDoc.ref.collection('Messages').orderBy('timestamp', 'desc').limit(1).onSnapshot(async (snapshot) => {
...
export async function getChat(chatID, addMessage) {
  ...
  // set up onSnapshot
  chat.messages = [];
  var messagesRef = await chatDoc.ref.collection('Messages').orderBy('timestamp', 'asc').get();
  chat.initMessages = messagesRef.docs.length;

  chat.unsubscribe = chatDoc.ref.collection('Messages').orderBy('timestamp', 'asc').onSnapshot(async (snapshot) => {
    if (snapshot.metadata.hasPendingWrites) return;
...
export async function sendChatMessage(chatID, msg) { // use only if chat already exists
  var chatRef = (await messagesCollection.where('id', '==', chatID).get());
...
export async function getUserNameMatches(name) {
  ...
  */
  ...
  for (var user of sendMessageUserCache) {
    if (user.fullName.toLowerCase().includes(name.toLowerCase())) matches.push(user);
  }
...
export async function createChat(users, message) { 
  ...
  */
  ...
  for (var user of users) {
    ...
    if (!sendMessageUserCache.find(val => val.id === user.id)) return "One or more users on the list do not exist.";
  }
  ...
  for (var chatID of userChatList) {
    ...
    var users1 = users.map(val => val.id).slice().sort();
    var users2 = chat.users.slice().sort();
    if (users1.join() == users2.join()) return "A chat with these users already exists.";
  }
