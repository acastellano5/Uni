...
export async function followUser(uid) {
  try {
    ...
    let userToFollowDoc =  (await db.where('uid', '==', uid).get()).docs
    const foundDoc = userToFollowDoc[0].ref
    console.log(userToFollowDoc);
    ...
    let curUser =  (await db.where('uid', '==', currentUser).get()).docs
    const curDoc = curUser[0].ref

    curUser= curUser[0]

    console.log(userToFollowDoc);
    const batcher = firestore().batch()
    batcher.update(foundDoc,
      {
        followers: firestore.FieldValue.arrayUnion(currentUser)
      }
    )
    batcher.update(curDoc,
      {
